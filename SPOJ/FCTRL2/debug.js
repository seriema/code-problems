...
function outputResult(result, test) {
	var ok = result.join('') === test.expected.toString();
	var okMsg = ('ok ' + test.bigint + ' * ' + test.smallint + ' = ' + test.expected.toString());
	var failMsg = ('NO! ' + test.bigint + ' * ' + test.smallint + ' was ' + result.toString() + ' which is not ' + test.expected.toString());
...
[
	...
	bi.addBigInt(big, test.smallint);
	outputResult(big, test);
});
...
[
	...
	var big = bi.createBigInt(test.bigint);
	bi.multiplyBigInt(big, test.smallint);
	outputResult(big, test);
});
